...
def parallelisedSearch(pattern, text):
    ...
    # creat n processes
    processes = [mp.Process(target=serialSearch, args=(pattern,text, worker_conn)) for i in range(n)]
    # start all processes        
    for p in processes:
        p.start()
    ...
    for i in range(n-1):
        master_conn.send([i*steps, (i+1)*steps])
    #send the last part of the text
    master_conn.send([(n-1)*steps, len(text)])
